...
class Solver:
    ...
    def unset_all(self):
        """Unmap all ciphertext symbol (map to '-')."""
        for C in ct_alphabet:
            self.alph[C] = '-'
    ...
    def repeated_patterns(self, l):
        """Report all patterns of length l and #occurrences > 1"""
        n = len(self.msg)
        dat = dict()
        pats = Counter(self.msg[i:i+l] for i in range(n - (l-1)))
...
class Shell(cmd.Cmd):
    ...
    def do_clear(self, arg):
        """Set all ciphertext symbol to - (unknown)"""
        self.core.unset_all()
    ...
    def do_patterns(self, arg):
        """patterns: find repeated patterns"""
        self.core.repeated_patterns(int(arg))
